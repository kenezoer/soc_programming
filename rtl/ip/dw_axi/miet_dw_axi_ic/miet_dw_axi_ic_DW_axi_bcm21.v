...
//
// Filename    : DW_axi_bcm21.v
// Revision    : $Id: //dwh/DW_ocb/DW_axi/amba_dev/src/DW_axi_bcm21.v#7 $
// Author      : Doug Lee    2/20/05
// Description : DW_axi_bcm21.v Verilog module for DW_axi
//
// DesignWare IP ID: fadce38c
//
////////////////////////////////////////////////////////////////////////////////



module miet_dw_axi_ic_DW_axi_bcm21 (
    clk_d,
    rst_d_n,
    data_s,
    data_d
...
`endif
    ...
    assign data_s_sel_0 = (clk_d_stopped==1'b1) ? data_s : data_s_sel_tmp;
    ...
    `ifdef DWC_BCM_SNPS_ASSERT_ON
    `ifdef DWC_BCM_CDC_COVERAGE_REPORT
    always@(data_s_int) begin
      if((data_s_int==$sampled(data_s)) && (!clk_d_stopped))
        -> data_s_ms_vld;
      end

      property data_s_missample_cov;
         @(data_s_ms_vld) disable iff (clk_d_stopped
                                       || !rst_d_n
                                         )
         ($sampled(clk_d_stopped)==1'b0);
      endproperty
      COVER_DATA_S_MISSAMPLE: cover property (data_s_missample_cov);
    `endif
    `endif
...
`endif
    ...
    assign data_s_sel_tmp = (data_s & ~data_select) | (last_data_dyn & data_select);
    assign data_s_int = (clk_d_stopped==1'b1) ? data_s : data_s_sel_tmp;
  `ifdef DWC_BCM_SNPS_ASSERT_ON
  `ifdef DWC_BCM_CDC_COVERAGE_REPORT
  always@(data_s_int) begin
    if((data_s_int==$sampled(data_s)) && (!clk_d_stopped))
...
// { START Latency Accurate modeling
  ...
  #`miet_dw_axi_ic_DW_HOLD_MUX_DELAY clk_d_q = 1'b1;

  always @ (negedge clk_d)
  #`miet_dw_axi_ic_DW_HOLD_MUX_DELAY clk_d_q = 1'b0;
  
  //Delay the source data
  assign #`miet_dw_axi_ic_DW_SETUP_MUX_DELAY data_s_q = (!rst_d_n) ? {WIDTH{1'b0}}:data_s;
...
`ifdef DWC_BCM_CDC_COVERAGE_REPORT
  generate if (SVA_TYPE_BIT1BIT0 == 0) begin : CDC_COVERAGE_REPORT
    reg clk_d_mod;
    reg rst_n_mod;
...
`endif
  ...
  generate
    if (SVA_TYPE_BIT1BIT0 == 0) begin : GEN_SVATP_EQ_0
    `ifdef DW_MODEL_MISSAMPLES
      DW_axi_bvm02
        U_CLK_DET (
        .clk         (clk_d        ),
    ...
    if (SVA_TYPE_BIT1BIT0 == 1) begin : GEN_SVATP_EQ_1
      DW_axi_bvm02
        U_CLK_DET (
        .clk         (clk_d        ),
        .rst_n       (rst_d_n      ),
      ...
      DW_axi_sva01 #(WIDTH, (F_SYNC_TYPE & 7), VERIF_EN) P_SYNC_HS (.*);
    end

    if (SVA_TYPE_BIT1BIT0 == 2) begin : GEN_SVATP_EQ_2
    `ifdef DW_MODEL_MISSAMPLES
      DW_axi_bvm02
        U_CLK_DET (
        .clk         (clk_d        ),
    ...
    `endif
      ...
      DW_axi_sva05 #(WIDTH, (F_SYNC_TYPE & 7)) P_SYNC_GC (.*);
    end

    if (SVA_TYPE_BIT2 == 0) begin : GEN_SVATP_BIT2_EQ_0
      DW_axi_sva08 #(WIDTH) P_SYNC_EQ_RST (.*);
    end
  endgenerate
  `else
    `ifdef DW_MODEL_MISSAMPLES
  DW_axi_bvm02 U_CLK_DET (
    .clk         (clk_d        ),
    .rst_n       (rst_d_n      ),
    .clk_stopped (clk_d_stopped),
    .clk_period  (clk_d_period )

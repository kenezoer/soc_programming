...
module i_axi_x2p_DW_axi_x2p_s_packer (/*AUTOARG*/
   // Outputs
   axi_data, 
                            axi_id, 
                            axi_resp, 
   ...
   input                                clk;
   input                                rstn;
   input [`i_axi_x2p_X2P_APB_DATA_WIDTH-1:0]      apb_data;
   // spyglass disable_block W240
   // SMD: An input has been declared but is not read.
   // SJ : apb_addr is read only when X2P_AXI_DW > X2P_APB_DATA_WIDTH.
   input [`i_axi_x2p_X2P_APB_ADDR_WIDTH-1:0]      apb_addr;
   // spyglass enable_block W240
   input                                clr_reg;
   
   
   input [`i_axi_x2p_X2P_AXI_SIDW-1:0]            cmd_id;
   input                                error;
   input                                enable_pack;
   input                                last;
   
   
   output [`i_axi_x2p_X2P_AXI_DW-1:0]             axi_data;
   output [`i_axi_x2p_X2P_AXI_SIDW-1:0]           axi_id;
   output                               axi_resp;
   output                               axi_last;
   
   wire [`i_axi_x2p_X2P_AXI_DW-1:0]           axi_data;   
  ...
  // In this config the buffers can be configured to single entry deep. Waiving off the unused signals in this configuration.
   reg [`i_axi_x2p_X2P_AXI_DW-1:0]            axi_data_reg, next_axi_data;
   reg [`i_axi_x2p_X2P_AXI_SIDW-1:0]          axi_id;
   wire                             axi_resp;
...
//   reg [`i_axi_x2p_X2P_APB_DATA_WIDTH-1:0] temp_data;
   
   
 /* AUTO_CONSTANT (`i_axi_x2p_X2P_APB_DATA_WIDTH, `i_axi_x2p_APB_BUS_SIZE, `i_axi_x2p_X2P_AXI_DW, `i_axi_x2p_X2P_MAX_AXI_SIZE) */ 
...
// go down all APB word wide inputs to the AXI word 
   ...
   always @(*)
     begin: NEXT_AXI_DATA_PROC
       start = 0;
       if (clr_reg) next_axi_data = {`i_axi_x2p_X2P_AXI_DW{1'b1}};//-1;
       else next_axi_data = axi_data_reg;
//temp_data = {`i_axi_x2p_X2P_APB_DATA_WIDTH{1'b0}};

       if (enable_pack)
         begin
           start = 0;     
           // each APB Data Word
           for (ii=0; ii < (`i_axi_x2p_X2P_AXI_DW/`i_axi_x2p_X2P_APB_DATA_WIDTH); ii = ii + 1)
             begin
             if (ii == 0) 
             // this is the one to update
             begin
// $display("----------%0t  start %0d AXI_DATA_WIDTH %0d ---------------------------------------",
// $time,start,`i_axi_x2p_X2P_AXI_DW);
       // spyglass disable_block W415a
       // SMD: Signal may be multiply assigned (beside initialization) in the same scope.
       // SJ : Here we are initializing the next_axi_data before assignment based on assertion of clr_reg signal.          
               for (jj=0; jj<`i_axi_x2p_X2P_APB_DATA_WIDTH; jj = jj + 1)
                 begin
...
//                           $time,ii,apb_axi_displ,start, next_axi_data, apb_data, temp_data);
               ...
               start = start + `i_axi_x2p_X2P_APB_DATA_WIDTH;
             end // for (ii=0; ii < (`i_axi_x2p_X2P_AXI_DW/`i_axi_x2p_X2P_APB_DATA_WIDTH); ii = ii + 1)
         end // if (enable_pack)
     end // always @ (...
   ...
   always @(posedge clk or negedge rstn)
     begin: S_AXI_REG_PROC
       if (rstn == 1'b0)
         begin
           axi_data_reg <= {`i_axi_x2p_X2P_AXI_DW{1'b0}};
           axi_id <= {`i_axi_x2p_X2P_AXI_SIDW{1'b0}};     
         end
       else
         begin

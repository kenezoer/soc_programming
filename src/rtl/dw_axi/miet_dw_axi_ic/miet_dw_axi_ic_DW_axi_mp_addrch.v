...
module miet_dw_axi_ic_DW_axi_mp_addrch (
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter PYLD_M_W = `miet_dw_axi_ic_AXI_AR_PYLD_M_W;
  // Channel payload width to slave.
  parameter PYLD_S_W = `miet_dw_axi_ic_AXI_AR_PYLD_S_W;
...
//----------------------------------------------------------------------
  ...
  input [`miet_dw_axi_ic_PAYLOAD_W-1:0]  payload_i;  // Channel payload vector.
  ...
  input [`miet_dw_axi_ic_ID_W-1:0]      cpl_id_i; // completion signals.
  ...
  //output [`miet_dw_axi_ic_AXI_REGIONW-1:0]   region_o;
  ...
  reg [`miet_dw_axi_ic_ID_W-1:0]      cpl_id_r; // signals, for timing considerations.
  ...
  reg [`miet_dw_axi_ic_AXI_MAX_NUM_MST_SLVS-1:0] xdcd_slvnum_oh;
  ...
  wire [`miet_dw_axi_ic_ID_W-1:0]     id_mst;   // ID and address signals from master.
  wire [`miet_dw_axi_ic_AXI_AW-1:0]   addr_mst;

  // Wires for unconnected sub module ports.
  wire [`miet_dw_axi_ic_AXI_LOG2_NSP1-1:0] sys_slv_unconn;
  wire [`miet_dw_axi_ic_AXI_MAX_NUM_USR_MSTS-1:0] bidi_sys_pnum_oh_unconn;
  ...
  wire valid_lp;
  wire [`miet_dw_axi_ic_AXI_REGIONW-1:0]   region_o;
 ...
 // wire [LOG2_MAX_UIDA*MAX_UIDA*MAX_CA_ID-1:0] act_ids_buffer; // buffer for active ids on address channel
  ...
  assign addr_mst
    = payload_i[`miet_dw_axi_ic_AXI_ARPYLD_ADDR_LHS:`miet_dw_axi_ic_AXI_ARPYLD_ADDR_RHS];

  assign id_mst
    = payload_i[`miet_dw_axi_ic_AXI_AWPYLD_ID_RHS_M+`miet_dw_axi_ic_ID_W-1:`miet_dw_axi_ic_AXI_AWPYLD_ID_RHS_M];


  wire [ `miet_dw_axi_ic_PAYLOAD_W-1:0]  payload_i_q;  // Channel payload vector.
  assign payload_i_q = payload_i;
  //--------------------------------------------------------------------
  // Instantiate the address decoder block.
  //--------------------------------------------------------------------
  miet_dw_axi_ic_DW_axi_dcdr
  ...
  assign local_slv_mux = `miet_dw_axi_ic_AXI_HAS_XDCDR
                         ? local_slv_xdcdr
                            : local_slv_idcdr;
 ...
 //   (`miet_dw_axi_ic_AXI_HAS_TZ_SUPPORT & (local_slv_mux != local_slv_tzone))
 //   ? (SHARED_S0 == 1'b1)
 //   : `miet_dw_axi_ic_AXI_HAS_XDCDR
 //     ? slv_on_shrd_xdcdr
 //     : slv_on_shrd_idcdr;
  ...
  assign slv_on_shrd_mux = (`miet_dw_axi_ic_AXI_HAS_TZ_SUPPORT
    )
    ?
    (SHARED_S0 == 1'b1)
    : `miet_dw_axi_ic_AXI_HAS_XDCDR
  ...
  //assign local_slv = `miet_dw_axi_ic_AXI_HAS_TZ_SUPPORT
  //                   ? local_slv_tzone
  //                     : local_slv_mux;
  ...
  begin : completion_regs_PROC
    if(!aresetn_i) begin
      cpl_tx_r <= 1'b0;
      cpl_id_r <= {`miet_dw_axi_ic_ID_W{1'b0}};
    end else begin
      cpl_tx_r <= cpl_tx_i;
      cpl_id_r <= cpl_id_i;
    end
  ...
  miet_dw_axi_ic_DW_axi_mp_idmask
  ...
  #(
    ...
    `miet_dw_axi_ic_S0_VIS, `miet_dw_axi_ic_S1_VIS, `miet_dw_axi_ic_S2_VIS, `miet_dw_axi_ic_S3_VIS, `miet_dw_axi_ic_S4_VIS, `miet_dw_axi_ic_S5_VIS, `miet_dw_axi_ic_S6_VIS,
    `miet_dw_axi_ic_S7_VIS, `miet_dw_axi_ic_S8_VIS, `miet_dw_axi_ic_S9_VIS, `miet_dw_axi_ic_S10_VIS, `miet_dw_axi_ic_S11_VIS, `miet_dw_axi_ic_S12_VIS, `miet_dw_axi_ic_S13_VIS,
    `miet_dw_axi_ic_S14_VIS, `miet_dw_axi_ic_S15_VIS, `miet_dw_axi_ic_S16_VIS,
  ...
  generate 
   ...
   begin
    ...
    begin : append_mstnum_to_id_PROC
      ...
      sidw = {MS_NUM,
              payload_i_q[`miet_dw_axi_ic_AXI_ARPYLD_ID_LHS_M:`miet_dw_axi_ic_AXI_ARPYLD_ID_RHS_M]};
      ...
      for(pyld_bit=0 ;
       ...
       if(pyld_bit<`miet_dw_axi_ic_AXI_ARPYLD_ID_RHS_M) begin
          payload_o_int[pyld_bit] = payload_i_q[pyld_bit];

        // Assign id field.
        end else if(   (pyld_bit>=`miet_dw_axi_ic_AXI_ARPYLD_ID_RHS_M)
                     && (pyld_bit<=(`miet_dw_axi_ic_AXI_ARPYLD_ID_LHS_M+`miet_dw_axi_ic_AXI_LOG2_NM))
                     )
        begin
        ...
        // SJ : The expression indexing the vector/array will never exceed the bound of the vector/array. 
            payload_o_int[pyld_bit] = payload_i_q[pyld_bit-`miet_dw_axi_ic_AXI_LOG2_NM];

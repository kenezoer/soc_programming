...
module miet_dw_axi_ic_DW_axi_sp_lockarb (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  `define miet_dw_axi_ic_IMPLEMENT_LOCKING ((LOCKING==1) && (NC>1))
...
//----------------------------------------------------------------------
  ...
  // lock_other_i, gated off when locking not implemented.
  assign lock_other_msk = `miet_dw_axi_ic_IMPLEMENT_LOCKING ? lock_other_i : 1'b0;
  ...
  generate
   if (`miet_dw_axi_ic_IMPLEMENT_LOCKING)
   begin
    if (AWCH)
    begin
     always @(*) 
  ...
  assign locktx = `miet_dw_axi_ic_IMPLEMENT_LOCKING 
                  ? ((rlocktx & rreq_i) 
                     | (wlocktx & wreq_i)
                    )
                  : {NC{1'b0}};
  ...
  // this channel. 
  assign outstnd_txs = 
    (`miet_dw_axi_ic_IMPLEMENT_LOCKING==1)
  ...
  assign outstnd_txs_fed_o = (`miet_dw_axi_ic_IMPLEMENT_LOCKING==1)
                             ? (outstnd_txs_r && (!outstnd_txs))
                             : 1'b0;
  ...
  assign outstnd_txs_nonlkd = (`miet_dw_axi_ic_IMPLEMENT_LOCKING==1) 
                              ? (outstnd_txs_nonlkd_o
                                  || outstnd_txs_nonlkd_i
                                )
                              : 1'b0;
  ...
  assign unlocking_tx_rcvd_r_jstfd = (`miet_dw_axi_ic_IMPLEMENT_LOCKING==1)
                                     ? unlocking_tx_rcvd_r
                                     : 1'b0;


  // Unlocking transaction received on either address channel.
  assign unlocking_tx_rcvd = `miet_dw_axi_ic_IMPLEMENT_LOCKING
  ...
  assign grant_pre_mask = 
    ( (    (this_ch_locked | locktx_granted)
      ...
      ? req_granted & (   grant_arb
                        | this_ch_locked 
                        | locktx_granted
                        | (`miet_dw_axi_ic_AXI_HAS_LOCKING == 0)
                      )
      : grant_arb
    );
  ...
  //spyglass enable_block FlopEConst
  //spyglass enable_block STARC05-2.2.3.3
  assign this_ch_locked = `miet_dw_axi_ic_IMPLEMENT_LOCKING ? this_ch_locked_r : 1'b0;
  ...
  assign both_ch_locked = `miet_dw_axi_ic_IMPLEMENT_LOCKING ? both_ch_locked_r : 1'b0;

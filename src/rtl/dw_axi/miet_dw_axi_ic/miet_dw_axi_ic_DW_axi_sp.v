...
module miet_dw_axi_ic_DW_axi_sp (
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  localparam BUS_AR_PYLD_S_W = AR_NVM*`miet_dw_axi_ic_AXI_AR_PYLD_S_W; 

  // Width of concatenated write address channel payload vector for all
  // visible master ports.
  localparam BUS_AW_PYLD_S_W = AW_NVM*`miet_dw_axi_ic_AXI_AW_PYLD_S_W; 

  // Width of concatenated write data channel payload vector for all
  // visible master ports.
  localparam BUS_W_PYLD_S_W = W_NVM*`miet_dw_axi_ic_AXI_W_PYLD_S_W;   

  // Width of concatenated master priorities for all visible master 
  // ports.
  localparam AR_BUS_PRIORITY_W = `miet_dw_axi_ic_AXI_MST_PRIORITY_W*AR_NVM;
  localparam AW_BUS_PRIORITY_W = `miet_dw_axi_ic_AXI_MST_PRIORITY_W*AW_NVM;
  localparam W_BUS_PRIORITY_W  = `miet_dw_axi_ic_AXI_MST_PRIORITY_W*W_NVM;
  
  localparam RPAYLOAD_W = `miet_dw_axi_ic_AXI_R_PYLD_M_W 
                          + (`miet_dw_axi_ic_AXI_LOG2_NM*`miet_dw_axi_ic_AXI_IS_ICM_M1);

  localparam BPAYLOAD_W = `miet_dw_axi_ic_AXI_B_PYLD_M_W 
                          + (`miet_dw_axi_ic_AXI_LOG2_NM*`miet_dw_axi_ic_AXI_IS_ICM_M1);
...
//----------------------------------------------------------------------
  ...
  output                        arvalid_o;   // Valid to external 
                                             // slave.
  output [`miet_dw_axi_ic_AXI_AR_PYLD_S_W-1:0] arpayload_o; // Payload to external 
                                             // slave.
  ...
  input [`miet_dw_axi_ic_AXI_R_PYLD_S_W-1:0]  rpayload_i; // Payload from external 
                                           // slave.
  ...
  output [`miet_dw_axi_ic_AXI_AW_PYLD_S_W-1:0] awpayload_o; // Payload to external 
                                             // slave.
  ...
  output [`miet_dw_axi_ic_AXI_W_PYLD_S_W-1:0]  wpayload_o; // Payload vector to 
                                            // external slave.
  ...
  input [`miet_dw_axi_ic_AXI_B_PYLD_S_W-1:0]  bpayload_i; // Payload from external 
                                           // slave.
  ...
  generate
    if(REMOVE_AR == 0) begin : gen_ar_addrch
      ...
      //----------------------------------------------------------------
      miet_dw_axi_ic_DW_axi_sp_addrch
      ...
      #(AR_NVM,             // Number of connected master ports.
        ...
        LOG2_NUM_VIS_MP,    // 
        `miet_dw_axi_ic_AXI_AR_PL_ARB,     // Pipeline AR channel arbiter ?
        ...
        `miet_dw_axi_ic_AXI_AR_PYLD_S_W,   // Width of payload vector to slave.
        ...
        `miet_dw_axi_ic_AXI_NOT_AW_CH      // Not a write address channel.
       )
     ...
     assign arpayload_o= {`miet_dw_axi_ic_AXI_AR_PYLD_S_W{1'b0}};
  ...
  //--------------------------------------------------------------------
  miet_dw_axi_ic_DW_axi_sp_drespch
  
  #(NUM_VIS_MP,         // Number of visible master ports.
    LOG2_NUM_VIS_MP,    // Log 2 of number of visible master ports.
    `miet_dw_axi_ic_AXI_R_PYLD_M_W,    // Payload width to master.
    `miet_dw_axi_ic_AXI_R_PYLD_S_W,    // Payload width from slave.
    ...
    `miet_dw_axi_ic_AXI_R_CH,          // R or B channel ?
  ...
  #(`miet_dw_axi_ic_AXI_R_TMO,        // Channel timing option.
    NUM_VIS_MP,        // Number of visible master ports.
    `miet_dw_axi_ic_AXI_R_PL_ARB,     // Is channel arbiter pipelined.
  ...
  generate
    if(REMOVE_AW == 0) begin : gen_aw_addrch
      ...
      #(AW_NVM,                // Number of connected master ports.
        ...
        NUM_VIS_MP,            // Number of visible master ports.
        LOG2_NUM_VIS_MP,       // 
        `miet_dw_axi_ic_AXI_AW_PL_ARB,        // Pipeline AW channel arbiter.
        ...
        BUS_AW_PYLD_S_W,       // Width of bus containing payloads 
                               // from all visible master ports.
        `miet_dw_axi_ic_AXI_AW_PYLD_S_W,      // Width of payload vector to slave.
        ...
        AW_BUS_PRIORITY_W,     // Width of bus containing priorities of 
                               // all visible masters.
        LOCKING,               // Implement locking or not.      
        `miet_dw_axi_ic_AXI_AW_CH,            // Write address channel.
      ...
      assign awpayload_o         = {`miet_dw_axi_ic_AXI_AW_PYLD_S_W{1'b0}};
  ...
  generate
    if(REMOVE_W == 0) begin : gen_w_datach
      ...
      //----------------------------------------------------------------
      miet_dw_axi_ic_DW_axi_sp_wdatach
      ...
      #(W_NVM,                 // Number of connected master ports and 
        ...
        AW_NVM,                // Num masters visible to dedicated AW
                               // channel.
        `miet_dw_axi_ic_AXI_W_PL_ARB,         // Pipeline W channel arbiter.
        ...
        BUS_W_PYLD_S_W,        // Width of bus containing payloads 
                               // from all visible master ports.
        `miet_dw_axi_ic_AXI_W_PYLD_S_W,       // Payload width to slave. 
        MAX_FAWC,              // Max. number of active write commands.
        LOG2_MAX_FAWC,       
      ...
      assign wpayload_o           = {`miet_dw_axi_ic_AXI_W_PYLD_S_W{1'b0}};
  ...
  #(NUM_VIS_MP,         // Number of visible master ports.
    LOG2_NUM_VIS_MP,    // Log 2 of number of visible master ports.
    `miet_dw_axi_ic_AXI_B_PYLD_M_W,    // Payload width to master.
    `miet_dw_axi_ic_AXI_B_PYLD_S_W,    // Payload width to slave.
    ...
    `miet_dw_axi_ic_AXI_NOT_R_CH,      // R or B channel ?
  ...
  #(`miet_dw_axi_ic_AXI_B_TMO,        // Channel timing option.
    NUM_VIS_MP,        // Number of visible master ports.
    `miet_dw_axi_ic_AXI_B_PL_ARB,     // Is channel arbiter pipelined.

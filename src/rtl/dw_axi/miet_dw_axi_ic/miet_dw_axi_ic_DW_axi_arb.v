...
module miet_dw_axi_ic_DW_axi_arb (
  // Inputs - System.
  aclk_i,
  aresetn_i,
  bus_priorities_i,
...
//----------------------------------------------------------------------
  ...
  localparam MASTERS_PRIORITY_W =(ARB_TYPE==4)? `miet_dw_axi_ic_AXI_QOSW: PRIORITY_W;

//----------------------------------------------------------------------
// LOCAL MACROS.
//----------------------------------------------------------------------
  `define miet_dw_axi_ic_ARB_PARK_INDEX    0 // Macro for park index parameter of
                              // instantiated designware arbiter.
            
  `define miet_dw_axi_ic_ARB_OP_MODE_NOREG 0 // Macro for output mode parameter of
  ...
  `define miet_dw_axi_ic_NC_INTERNAL  ((NC==1) ? 2 : NC)            

  // Macro for priority bus to the arbiter, using NC_INTERNAL macro.
  `define miet_dw_axi_ic_BUS_PRIOR_W_INTERNAL (`miet_dw_axi_ic_NC_INTERNAL*MASTERS_PRIORITY_W)
...
// Outputs - Channel Destination.
  ...
  wire [`miet_dw_axi_ic_NC_INTERNAL-1:0] bus_grant_arb_o;   //
  ...
  // for these signals.
  wire [`miet_dw_axi_ic_NC_INTERNAL-1:0]          req_arb;
  wire [`miet_dw_axi_ic_BUS_PRIOR_W_INTERNAL-1:0] bus_priorities_inv_arb;

  wire arb_enable; // Signal to disable/stall the arbiters.


  // Multi cycle arbitration signals.
  wire [`miet_dw_axi_ic_NC_INTERNAL-1:0] req_mca; // Request signals from multi cycle
  ...
  wire [`miet_dw_axi_ic_BUS_PRIOR_W_INTERNAL-1:0] bus_priorities_mca;
  ...
  wire parked_unconn;
    // Pass 2 bits if NC == 1.
  wire [`miet_dw_axi_ic_NC_INTERNAL-1:0] use_other_pri_int; 
  //wire [`miet_dw_axi_ic_NC_INTERNAL-1:0] bus_grant_lock_int;
  //Not used if NC==1 
  wire [(`miet_dw_axi_ic_NC_INTERNAL*ARB_INDEX_W)-1:0] bus_priority_other_int;
  wire [(`miet_dw_axi_ic_NC_INTERNAL*ARB_INDEX_W)-1:0] bus_priority_o_int;
  //--------------------------------------------------------------------
  ...
  generate 
    ...
    begin
     ...
     assign bus_priority_other_int = {(`miet_dw_axi_ic_NC_INTERNAL*ARB_INDEX_W){1'b0}};
    end
  endgenerate   
 ...
 //VP:: To avoid lint error assigned unused bits to 0
  generate
    if (`miet_dw_axi_ic_BUS_PRIOR_W_INTERNAL == BUS_MASTERS_PRIORITY_W) 
      assign bus_priorities_inv_arb = bus_priorities_inv;
    else
      assign bus_priorities_inv_arb = {{(`miet_dw_axi_ic_BUS_PRIOR_W_INTERNAL -BUS_MASTERS_PRIORITY_W){1'b0}}, bus_priorities_inv};
  endgenerate  
  ...
  */
  ...
  //--------------------------------------------------------------------
  miet_dw_axi_ic_DW_axi_mca_reqhold
  
  #(MCA_EN,                // 1 if multi cycle arbitration is enabled.
    MCA_HLD_PRIOR,         // 1 if priority bits should be registered.
    `miet_dw_axi_ic_BUS_PRIOR_W_INTERNAL, // Width if priority signal bus.
    `miet_dw_axi_ic_NC_INTERNAL,          // Number of request signals.
  ...
  generate 
    case (ARB_TYPE)
      ...
      `miet_dw_axi_ic_AXI_ARB_TYPE_DP : begin : gen_arb_type_dp
        
      assign bus_priority_o_int = {`miet_dw_axi_ic_NC_INTERNAL*ARB_INDEX_W{1'b0}};
        miet_dw_axi_ic_DW_axi_arbiter_dp
        
        #(`miet_dw_axi_ic_NC_INTERNAL,       // Number of clients.
          MASTERS_PRIORITY_W,         // Priority signal width per client.   
          ARB_PARK_MODE,     
          `miet_dw_axi_ic_ARB_PARK_INDEX,    
          `miet_dw_axi_ic_ARB_OP_MODE_NOREG,
        ...
        U_DW_axi_arbiter_dp (
          ...
          .request      (req_mca),
          .lock         ({`miet_dw_axi_ic_NC_INTERNAL{1'b0}}), // Not using lock inputs.
          .mask         ({`miet_dw_axi_ic_NC_INTERNAL{1'b0}}), // Not using mask inputs.
      ...
      `miet_dw_axi_ic_AXI_ARB_TYPE_FCFS : begin : gen_arb_type_fcfs
        miet_dw_axi_ic_DW_axi_arbiter_fcfs
      ...
      `miet_dw_axi_ic_AXI_ARB_TYPE_2T : begin : gen_arb_type_2t
        miet_dw_axi_ic_DW_axi_arbiter_fae
 ...
 `miet_dw_axi_ic_AXI_ARB_TYPE_QOS : begin : gen_arb_type_qos
  ...
  miet_dw_axi_ic_DW_axi_busdemux

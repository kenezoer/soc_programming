...
module miet_dw_axi_ic_DW_axi_sp_wdatach (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter BUS_PYLD_M_W = (2*`miet_dw_axi_ic_AXI_W_PYLD_M_W); 

  parameter PYLD_S_W = `miet_dw_axi_ic_AXI_W_PYLD_M_W; // Payload width to slave. 
...
//----------------------------------------------------------------------
  ...
  `define miet_dw_axi_ic_ARB_NC ((NUM_CON_MP==1) ? 1 : (SHARED ? NUM_CON_MP : WID))

  // Log base 2 of number of clients to the arbiter.
  `define miet_dw_axi_ic_ARB_LOG2_NC ((NUM_CON_MP==1) ? 1 : (SHARED ? LOG2_NUM_CON_MP : LOG2_WID))
  
  // Log base 2 of (number of clients to the arbiter + 1).
  `define miet_dw_axi_ic_ARB_LOG2_NC_P1 ((NUM_CON_MP==1) ? 1 : (SHARED ? LOG2_NUM_CON_MP_P1 : LOG2_WID_P1))

  // Width of priorities bus from the wrorder block, priorities for
  // 0 to WID-1.
  `define miet_dw_axi_ic_BUS_PRIORITY_WID_W (WID*`miet_dw_axi_ic_AXI_MST_PRIORITY_W)

  // Width of bus containing master numbers of masters which are
  // being allowed to access the arbiter.
  `define miet_dw_axi_ic_BUS_PORT_REQ_W (WID*LOG2_NUM_CON_MP)
  ...
  `define miet_dw_axi_ic_USE_INT_GRANT_INDEX 0

  `define miet_dw_axi_ic_REG_INTER_BLOCK_PATHS 0

  // Width of arbiter internal grant index.                                
  localparam ARB_INDEX_W = (ARB_TYPE==1) 
                           ? `miet_dw_axi_ic_ARB_LOG2_NC_P1 
                           : `miet_dw_axi_ic_ARB_LOG2_NC;
...
//----------------------------------------------------------------------
  ...
  wire [`miet_dw_axi_ic_BUS_PRIORITY_WID_W-1:0] bus_priorities_wrorder;            

  // Bus containing master numbers who are allowed to request to the
  // arbiter.
  wire [`miet_dw_axi_ic_BUS_PORT_REQ_W-1:0] bus_mst_req_wrorder;
  reg [`miet_dw_axi_ic_BUS_PORT_REQ_W-1:0] bus_mst_req_wrorder_r; // Reg'd version.
  wire [`miet_dw_axi_ic_BUS_PORT_REQ_W-1:0] bus_mst_req_pla_mux; // Selected from above
                                                  // 2 depending on 
                                                  // pipelining.
  ...
  wire [`miet_dw_axi_ic_ARB_NC-1:0] req_arb; // Selected from bus_valid_i[0]
                                       // or req_wrorder depending on
                                       // number of visible master
                                       // ports.

  reg [`miet_dw_axi_ic_ARB_NC-1:0] req_arb_r; // registered version of 
                                        // req_arb.
           
  wire [`miet_dw_axi_ic_ARB_NC-1:0] req_arb_mux; // Selected from req_arb
                                           // and req_arb_r depending
                                           // on PL_ARB.

  // Selected from 0 or bus_priorities_wrorder, depending on number of
  // visible master ports.
  wire [`miet_dw_axi_ic_BUS_PRIORITY_WID_W-1:0] bus_priorities_arb;            

  // Selected from 0 or bus_mst_req_wrorder, depending on number of
  // visible master ports.
  wire [`miet_dw_axi_ic_BUS_PORT_REQ_W-1:0] bus_mst_req_arb;

  wire grant; // Grant output from DW_axi_arb.

  // Arbiter grant index.
  wire [`miet_dw_axi_ic_ARB_LOG2_NC-1:0] arb_grant_index; 
  // Arbiter 1 hot grant output.
  wire [`miet_dw_axi_ic_ARB_NC-1:0] arb_bus_grant; 
  ...
  wire [`miet_dw_axi_ic_AXI_MAX_NUM_USR_MSTS-1:0] aw_shrd_param_bus;
  wire [`miet_dw_axi_ic_AXI_MAX_NUM_USR_MSTS-1:0] w_shrd_param_bus;
  ...
  wire [`miet_dw_axi_ic_AXI_MAX_NUM_USR_MSTS-1:0] vis_bus;
  ...
  wire [`miet_dw_axi_ic_AXI_MAX_NUM_MST_SLVS-1:0] has_ddctd_s_bus_max;
  // Above sized for NSS.
  ...
  wire [(`miet_dw_axi_ic_ARB_NC*ARB_INDEX_W)-1:0] bus_priority_unconn;
  ...
  always @(*) begin : aw_shrd_param_bus_lcl_PROC
    ...
    for(sm=0;sm<`miet_dw_axi_ic_AXI_MAX_NUM_USR_MSTS;sm=sm+1) begin
      if(vis_bus[sm]) begin
  ...
  */
  ...
  assign issued_tx_mux = `miet_dw_axi_ic_REG_INTER_BLOCK_PATHS
                         ? issued_tx_r : issued_tx_c;

  assign issued_mstnum_mux = `miet_dw_axi_ic_REG_INTER_BLOCK_PATHS
                             ? issued_mstnum_r : issued_mstnum_c;

  assign issued_tx_shrd_slv_oh_mux 
  = `miet_dw_axi_ic_REG_INTER_BLOCK_PATHS
  ...
  generate 
    if(SHARED == 0) begin : gen_sp_wdatach_w_not_shared
      ...
      //----------------------------------------------------------------
      miet_dw_axi_ic_DW_axi_sp_wrorder
      ...
      #(NUM_CON_MP,           // Number of visible master ports.
        ...
        BUS_PRIORITY_W,       // Width of bus with priorities per 
                              // visible master.
        `miet_dw_axi_ic_BUS_PRIORITY_WID_W,  // Width of bus with priorities per write
                              // interleaving depth.
        `miet_dw_axi_ic_BUS_PORT_REQ_W,      // Width of bus containing master numbers 
                              // which
     ...
     assign    bus_mst_req_wrorder = {`miet_dw_axi_ic_BUS_PORT_REQ_W{1'b0}};
     assign    bus_priorities_wrorder = {`miet_dw_axi_ic_BUS_PRIORITY_WID_W{1'b0}};
    ...
    if(SHARED) begin : gen_sp_wdatach_w_shared
      ...
      //----------------------------------------------------------------
      miet_dw_axi_ic_DW_axi_sp_shrd_wrorder
  ...
  assign bus_mst_req_arb = (NUM_CON_MP==1) 
                           ?{`miet_dw_axi_ic_BUS_PORT_REQ_W{1'b0}}
                           : bus_mst_req_wrorder;

  assign bus_priorities_arb = (NUM_CON_MP==1)
                              ?{`miet_dw_axi_ic_BUS_PRIORITY_WID_W{1'b0}}
  ...
  // Channel Arbiter
  //--------------------------------------------------------------------
  miet_dw_axi_ic_DW_axi_arb
   
  #(ARB_TYPE,                 // Arbitration type.
    `miet_dw_axi_ic_ARB_NC,                  // Number of clients to arbiter.
    `miet_dw_axi_ic_ARB_LOG2_NC,             // Log base 2 of number of clients.
    `miet_dw_axi_ic_ARB_LOG2_NC_P1,          // Log base 2 of (number of clients + 1).
    ...
    `miet_dw_axi_ic_AXI_MST_PRIORITY_W,      // Priority width of a single master.            
    `miet_dw_axi_ic_BUS_PRIORITY_WID_W,      // Width of priorities bus.
  ...
  U_DW_axi_arb (
    ...
    .locktx_i             ({`miet_dw_axi_ic_ARB_NC{1'b0}}), // Not required here.
    .unlock_i             (1'b0), // Not required here.
    .grant_masked_i       (1'b0), // Not required here.

    // Not required here, next 2 inputs required on address
    // channels only.
    .use_other_pri_i      ({`miet_dw_axi_ic_ARB_NC{1'b0}}), 
    //.bus_grant_lock_i     ({`miet_dw_axi_ic_ARB_NC{1'b0}}),
  ...
  generate
    if(SHARED==0) begin : gen_sp_wdatach_dcd_not_shared
      ...
      begin : bus_mst_req_wrorder_r_PROC
        if(~aresetn_i) begin
          bus_mst_req_wrorder_r <= {`miet_dw_axi_ic_BUS_PORT_REQ_W{1'b0}};
        end else begin
      ...
      #(`miet_dw_axi_ic_ARB_NC,      // Number of input busses.
        LOG2_NUM_CON_MP,       // Width of each input bus.
        `miet_dw_axi_ic_ARB_LOG2_NC  // Width of select line.
      ) 
      U_port_req_mux (
        .sel  (arb_grant_index),
  ...
  #(`miet_dw_axi_ic_ARB_NC,     // Number of inputs to the mux.
    1,                    // Width of each input to the mux.
    `miet_dw_axi_ic_ARB_LOG2_NC // Width of select input for the mux.
  )
  U_DW_axi_busmux_mca (
    .sel  (arb_grant_index),
    .din  (req_arb_mux), 
  ...
  assign cpl_tx = payload_pre_irs[`miet_dw_axi_ic_AXI_WPYLD_LAST] & valid_o & ready_i;

  // Shared Channel
  // Detect write data part completion for each slave attached to the
  // shared channel.
  assign cpl_tx_shrd_bus = 
    (  {NSS{payload_pre_irs[`miet_dw_axi_ic_AXI_WPYLD_LAST]}}
  ...
  #(((SHARED & SHARED_PL) ? `miet_dw_axi_ic_AXI_TMO_FRWD : 0), // Channel timing option
  ...
  */
  ...
  `undef miet_dw_axi_ic_ARB_NC
  `undef miet_dw_axi_ic_ARB_LOG2_NC
  `undef miet_dw_axi_ic_ARB_LOG2_NC_P1
  `undef miet_dw_axi_ic_REG_INTER_BLOCK_PATHS 
